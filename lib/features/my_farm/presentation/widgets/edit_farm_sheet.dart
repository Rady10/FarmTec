...
class _EditFarmSheetState extends State<EditFarmSheet> {
  ...
  void initState() {
    ...
    _selectedCrop = widget.farm.crop;
    _areaCtrl.text = widget.farm.area.replaceAll(RegExp(r'\s*ha\s*', caseSensitive: false), '').trim();
    _latCtrl.text = widget.farm.lat == 0 ? '' : widget.farm.lat.toString();
    _lngCtrl.text = widget.farm.lng == 0 ? '' : widget.farm.lng.toString();
    _plantedAt = widget.farm.plantedAt ?? DateTime.now();
  }
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Text(
              l.tr('edit') == 'edit' ? 'Edit Farm' : '${l.tr('edit')} ${l.tr('my_farm').toLowerCase()}',
              style: AppFonts.font(
                fontSize: 22,
